...
module.exports.wipeCardData = card => {
    ...
    card.expiry = card.expiry.slice(0, 4) + '/' + card.expiry.slice(5, 7)
}
 ...
 *      - attaches boolean property to address object, isPrimaryAddress
 *
 ...
 */
 module.exports.attachIsPrimaryAddress = (address, primary_address_id) => {
    if (primary_address_id) {
        address.isPrimaryAddress = address.id === primary_address_id;
    } else {
 ...
 *      - attaches boolean property to payment object, isPrimaryPayment
 *
 ...
 */
 module.exports.attachIsPrimaryPayment = (payment, primary_payment_id) => {
    if (primary_payment_id) {
        payment.isPrimaryPayment = payment.id === primary_payment_id;
    } else {
        // base case if primary_payment_id is null 
        payment.isPrimaryPayment = false;
    }
 }