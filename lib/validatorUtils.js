 ...
 * @param {String} password password of user
 */
module.exports.validateAuthInputs = (email, password) => {
    validateEmail(email);
 ...
 *      - validate inputs to prevent XSS attack
 *      - Use for creating or updating an Cart Item object
 * Throws errors if not 
 * 
 ...
 */
 module.exports.validateAddressInputs = (data) => {
    ...
    data.isPrimaryAddress = (data.isPrimaryAddress === undefined) ? null : data.isPrimaryAddress ;
    ...
    if ( address1 === null        || typeof address1   !== 'string'  ||
        city === null            || typeof city       !== 'string'  ||
        state === null           || typeof state      !== 'string'  ||
        zip === null             || typeof zip        !== 'string'  ||
        ...
        (isPrimaryAddress !== null && (typeof isPrimaryAddress !== 'boolean')) ||
        (address2 !== null && (typeof address2 !== 'string'|| address2.length === 0 || address2.length > 30))    
    ){
        throw httpError(400, 'Invalid input.');
    }
}
...
module.exports.validatePaymentInputs = (data) => {
    // make sure card_type and isPrimaryPayment properties are defined 
    data.card_type = (data.card_type === undefined) ? null : data.card_type;
    data.isPrimaryPayment = (data.isPrimaryPayment === undefined) ? null : data.isPrimaryPayment ;

    const { 
        card_type,          // can be null
        provider,   
        card_no, 
        cvv,
        exp_month,
    ...
    if (provider === null                   || provider.length === 0                     || 
        ...
        exp_month === null                  || !validateValue(exp_month, 1, 12)          ||
        exp_year === null                   || !validateValue(exp_year, 2021, 2100)      ||
